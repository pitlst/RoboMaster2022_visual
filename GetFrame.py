...
class GetFrame:
    ...
    def SetCamera(self):
        if self.video_debug_set:
            ...
            if ret != 0:
                print ("set offsetX fail! ret[0x%x]" % ret)
                sys.exit()
            ret = self.cam.MV_CC_SetIntValue("OffsetY", 0)
            if ret != 0:
                print ("set offsetY fail! ret[0x%x]" % ret)
                sys.exit()
            ...
            if ret != 0:
                print ("set DeviceLinkThroughputLimitMode fail! ret[0x%x]" % ret)
                sys.exit()
            '''
            ...
            if ret != 0:
                print ("set RegionSelector fail! ret[0x%x]" % ret)
                sys.exit()
            '''
            ...
            if ret != 0:
                print ("set ReverseX fail! ret[0x%x]" % ret)
                sys.exit()
            '''
            设置相机传输的图像的格式
            ...
            if ret != 0:
                print ("set TestPattern fail! ret[0x%x]" % ret)
                sys.exit()
            #关闭帧水印
            ret = self.cam.MV_CC_SetBoolValue("FrameSpecInfo",self.FrameSpecInfo)
            if ret != 0:
                print ("set FrameSpecInfo fail! ret[0x%x]" % ret)
                sys.exit()
            ...
            #if ret != 0:
            #    print ("set TriggerCacheEnable fail! ret[0x%x]" % ret)
            #    sys.exit()
            #设置曝光或者快门的运行方式
            ret = self.cam.MV_CC_SetEnumValue("ExposureMode",self.ExposureMode)
            if ret != 0:
                print ("set ExposureMode fail! ret[0x%x]" % ret)
                sys.exit()
            '''        
            #设置曝光时间
            ...
            if ret != 0:
                print ("set HDREnable fail! ret[0x%x]" % ret)
                sys.exit()
            ...
            if ret != 0:
                print ("set BlackLevelEnable fail! ret[0x%x]" % ret)
                sys.exit()
            #设置黑电平调整参数
            ...
            if ret != 0:
                print ("set LineInverter fail! ret[0x%x]" % ret)
                sys.exit()
