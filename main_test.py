...
if sys.platform.startswith('win'):
    log.print_info('run computer is win and can not open HIVISION')
...
class Main:
    def __init__(self, command_input):
        ...
        self.GetArmor_class = GetArmor(
            self.debug, self.video_debug_set, self.color_init, self.mode_init)
        log.print_info('aimbot init done')
        if energy_label:
            #初始化深度学习识别类，需要debug模式标志位，数据输入标志位,颜色信息
            self.GetEnergyMac_class = GetEnergyMac(
                self.debug, self.video_debug_set, self.color_init)
            #初始化深度学习预测类，需要模式信息
        ...
        while 1:
            ...
            if type(frame) is np.ndarray:
                if reset_label in [0, 3]:
                    ...
                    msg_temp = list(
                        self.AnglePredicted_class.NormalHit(msg_temp, f_time))
            #串口发送消息
    ...
    def pyqt_show(self):
        ...
        if label:
            ...
            app.setStyleSheet(qdarkstyle.load_stylesheet())  # 更换样式表
            self.silder = Opencv_slider('Opencv_slider',self.mode_init,self.color_init)
            self.silder.show()
            app.exec()
        ...
        while label:
            ...
            if energy_label:
                ...
                for i, frame in enumerate(frame_set2):
                    if i == 3:
                        cv2.circle(frame, (int(msg_set[0]), int(
                            msg_set[1])), 5, (0, 255, 255), -1)
...
def stop_thread(th):
    ...
    res = ctypes.pythonapi.PyThreadState_SetAsyncExc(
        th_id, ctypes.py_object(SystemExit))
    if res == 0:
        raise ValueError("成功退出线程 序号："+str(th.ident))
    elif res != 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', '-d', action='store_true',
                        help='是否开启滑动条调参/debug模式,不输入默认不开启')
    parser.add_argument('--breakpoint', '-b',
                        action='store_true', help='是否开启断点模式,不输入默认不开启')
    parser.add_argument('--serial', '-c', action='store_true',
                        help='是否开启串口,不输入默认开启，输入后串口传入值固定从debug.json读取')
    parser.add_argument('--source', '-s', type=str, default='HIVISION',
                        help='输入识别数据的来源，可以是视频的路径或者图片文件夹的路径，不输入该选项默认海康摄像头，输入该选项默认usb摄像头')
